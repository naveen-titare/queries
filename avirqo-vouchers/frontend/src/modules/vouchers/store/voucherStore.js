...
export const useVoucherStore = defineStore('vouchers', {
  state: () => ({
    ...
    cart: JSON.parse(sessionStorage.getItem('avirqo_cart') || '[]'),
  }),
  ...
  actions: {
    async fetchCatalog(params = {}) {
      ...
      this.error = null;
      try {
        const { data } = await voucherApi.getCatalog(params);
        ...
        this.error = e.response?.data?.message || 'Failed to load catalog.';
      } finally {
        this.loading = false;
      }
    },
    ...
    async fetchProduct(id) {
      this.loading = true;
      try {
        const { data } = await voucherApi.getProduct(id);
    ...
    clearCart() {
      this.cart = [];
      sessionStorage.removeItem('avirqo_cart');
    },

    _saveCart() {
      sessionStorage.setItem('avirqo_cart', JSON.stringify(this.cart));
    },
    ...
    async validateCart() {
      ...
      return await voucherApi.validateCart(items);
    },
    ...
    async placeOrder(customerId, spocId) {
      ...
      const { data } = await voucherApi.placeOrder({ customer_id: customerId, spoc_id: spocId, items });
      this.clearCart();
      return data;
    },
    ...
    async fetchOrders(params = {}) {
      this.loading = true;
      try {
        const { data } = await voucherApi.getOrders(params);
